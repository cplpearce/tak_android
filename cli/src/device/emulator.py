...
class Emulator(Device):
    ...
    def __init__(
        ...
        self.path_device_profile_target = os.path.join(
            workdir, ".android", "devices.xml"
        )
        self.path_emulator = os.path.join(workdir, "emulator")
        self.path_emulator_config = os.path.join(workdir, "emulator", "config.ini")
        self.path_emulator_profiles = os.path.join(
            workdir, "docker-android", "mixins", "configs", "devices", "profiles"
        )
        self.path_emulator_skins = os.path.join(
            workdir, "docker-android", "mixins", "configs", "devices", "skins"
        )
    ...
    def is_initialized(self) -> bool:
        ...
        if os.path.exists(self.path_emulator_config):
            ...
            with open(self.path_emulator_config, "r") as f:
                if any(
                    re.match(r"hw\.device\.name ?= ?{}".format(self.device), line)
                    for line in f
                ):
    ...
    def _add_profile(self) -> None:
        if "samsung" in self.device.lower():
            path_device_profile_source = os.path.join(
                self.path_emulator_profiles, "{fn}.xml".format(fn=self.file_name)
            )
    ...
    def create(self) -> None:
        ...
        if first_run:
            ...
            self._add_profile()
            creation_cmd = (
                "avdmanager create avd -f -n {n} -b {it}/{si} "
    ...
    def change_permission(self) -> None:
        ...
        if os.path.exists(kvm_path):
            cmds = (f"sudo chown 1300:1301 {kvm_path}", "sudo sed -i '1d' /etc/passwd")
            for c in cmds:
    ...
    def check_adb_command(
        self,
        readiness_check_type: ReadinessCheck,
        bash_command: str,
        expected_keyword: str,
        max_attempts: int,
        interval_waiting_time: int,
        adb_action: str = None,
    ) -> None:
        success = False
            ...
            if success:
                ...
                try:
                    ...
                    if expected_keyword in str(output).lower():
                        ...
                        self.logger.info(
                            f"[attempt: {_}] {self.device_type} is not {readiness_check_type.value}! "
    ...
    def wait_until_ready(self) -> None:
        ...
        booting_cmd = (
            f"adb -s {self.adb_name} wait-for-device shell getprop sys.boot_completed"
        )
        focus_cmd = (
            f"adb -s {self.adb_name} shell dumpsys window | grep -i mCurrentFocus"
        )
        self.check_adb_command(
            self.ReadinessCheck.BOOTED, booting_cmd, "1", 60, self.interval_waiting
        )
        ...
        self.check_adb_command(
            self.ReadinessCheck.POP_UP_WINDOW,
            focus_cmd,
            pop_up_system_ui,
            max_attempt_pop_up,
            interval_pop_up,
        ...
        self.check_adb_command(
            self.ReadinessCheck.WELCOME_SCREEN,
            focus_cmd,
            "launcheractivity",
            60,
            self.interval_waiting,
        )
    ...
    def __repr__(self) -> str:
        try:
            return (
                "Emulator(name={n}, device={d}, adb_name={an}, android_version={av}, api_level={al}, "
                ...
                "file={f})".format(
                    ...
                    d=self.device,
                    an=self.adb_name,
                    av=self.android_version,
                    al=self.api_level,
                    dp=self.data_partition,
                    aa=self.additional_args,
                    it=self.img_type,
                    si=self.sys_img,
                    pdpt=self.path_device_profile_target,
                    pe=self.path_emulator,
                    pec=self.path_emulator_config,
                    f=self.file_name,
                )
            )
